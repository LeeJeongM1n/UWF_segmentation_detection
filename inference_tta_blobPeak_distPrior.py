...
def gaussian_prior(dist: float, mu: float, sigma: float) -> float:
    if sigma <= 1e-6:
        return 0.0
    z = (dist - mu) / sigma
    return float(np.exp(-0.5 * z * z))

def select_macula_with_prior(hm_mac: np.ndarray, thr: float, disc_xy: Tuple[int, int],
                             mu_px: float, sigma_px: float,
                             w_peak: float = 1.0, w_sum: float = 0.0):
    """
    Return (x,y,v) for macula chosen by blob_score * prior(distance_to_disc).
    blob_score default = peak (optionally combine sum/mean).
    """
    ...
    for c in cands:
        ...
        prior = gaussian_prior(dist, mu_px, sigma_px)

        # blob 자체 점수 
        blob_score = w_peak * c["v"] + w_sum * np.sqrt(max(c["sum"], 0.0))

        score = blob_score * prior
...
def draw_mac_top3_with_prior_score_on_bgr(
    bgr: np.ndarray,
    mac_cands: list,
    disc_xy: Optional[Tuple[int, int]],
    mu_px: float,
    sigma_px: float,
    topk: int = 3,
):
    ...
    # 1) blob_peak(v) 기준 top3
    mac_cands = sorted(mac_cands, key=lambda c: c["v"], reverse=True)[:topk]

    for i, c in enumerate(mac_cands):
        x, y, v = int(c["x"]), int(c["y"]), float(c["v"])
        cv2.circle(bgr, (x, y), 6, (0, 0, 255), 2)  # 빨간 원

        # 기본 라벨: M{i}(v)
        base_txt = f"M{i}({v:.2f})"
        cv2.putText(bgr, base_txt, (x + 8, y + 12), font, 0.5, (0, 0, 255), 1)

        # 2) prior score (빨간 글씨)
        if disc_xy is not None:
            dx = float(c["xc"]) - float(disc_xy[0])
            dy = float(c["yc"]) - float(disc_xy[1])
            dist = float(np.hypot(dx, dy))
            prior = gaussian_prior(dist, mu_px, sigma_px)
            score = v * prior
            score_txt = f"s={score:.3f}"
            # score는 '빨간 글씨' (요구사항)
            cv2.putText(bgr, score_txt, (x + 8, y + 30), font, 0.5, (0, 0, 255), 1)
...
def main():
    ...
    parser.add_argument("--tta_conf_thr", default=0, type=float)
    parser.add_argument("--tta_peak_spread_thr", default=0, type=float)
    ...
    # add distance prior
    parser.add_argument("--mac_prior_mu_px", type=float, required=True,
                    help="expected disc-macula distance (px) in resized image")
    parser.add_argument("--mac_prior_sigma_px", type=float, required=True,
                        help="std of disc-macula distance (px) in resized image")
    ...
    flip_dbg_dir = out_root / "flip_debug" 

    overlay_dir.mkdir(exist_ok=True)
    heatmap_dir.mkdir(exist_ok=True)
    binary_dir.mkdir(exist_ok=True)
    tta_dbg_dir.mkdir(exist_ok=True)
    flip_dbg_dir.mkdir(exist_ok=True)
    ...
    with torch.no_grad():
        for img_path in tqdm(list_images(Path(args.input_dir)), desc="Inference"):
            ...
            for t in range(preds.shape[0]):
                ...
                # m = find_blob_peak(preds[t, 1], args.blob_thr)

                if d is None:
                    m = find_blob_peak(preds[t, 1], args.blob_thr)
                else:
                    xd, yd, _ = d
                    m = select_macula_with_prior(
                        hm_mac=preds[t, 1],
                        thr=args.blob_thr,
                        disc_xy=(xd, yd),
                        mu_px=args.mac_prior_mu_px,
                        sigma_px=args.mac_prior_sigma_px,
                        w_peak=1.0,
                        w_sum=0.0,
                    )           
            ...
            for t in range(raw_preds.shape[0]):
                ...
                # debug (flipped input image + flipped heatmap without marker)
                if t in (1,2):
                    flip_in_path = flip_dbg_dir / f"{name}_{tag_rawdbg.get(t, f'T{t}')}_input.png"
                    cv2.imwrite(str(flip_in_path), cv2.cvtColor(view_rgb, cv2.COLOR_RGB2BGR))

                    flip_hm_path = flip_dbg_dir / f"{name}_{tag_rawdbg.get(t, f'T{t}')}_rawheatmap.png"
                    hm_bgr = both_pred_to_bgr_u8(raw_preds[t, 0], raw_preds[t, 1])
                    cv2.imwrite(str(flip_hm_path), hm_bgr)

                    flip_ov_path = flip_dbg_dir / f"{name}_{tag_rawdbg.get(t, f'T{t}')}_overlay.png"
                    heat_raw_rgb = heatmap_both_to_color_rgb(raw_preds[t, 0], raw_preds[t, 1])
                    over_raw = overlay(view_rgb, heat_raw_rgb, args.alpha)
                    cv2.imwrite(str(flip_ov_path), cv2.cvtColor(over_raw, cv2.COLOR_RGB2BGR))
                ...
                disc_xy = (d_raw[0], d_raw[1]) if d_raw is not None else None

                # macula (top 3 : blob_peak max value + gaussian score with distance prior)
                over_bgr = draw_mac_top3_with_prior_score_on_bgr(
                    over_bgr,
                    mac_cands=mac_cands,
                    disc_xy=disc_xy,
                    mu_px=args.mac_prior_mu_px,
            ...
            rows.append(
                {
                    ...
                    "threshold": float(args.threshold),
                    "blob_thr": float(args.blob_thr),
                    "tta_conf_thr": float(args.tta_conf_thr),
                    "tta_peak_spread_thr": float(args.tta_peak_spread_thr),
                    "Disc_conf": disc_conf,
                    "Disc_peak_var": disc_spread,  # px spread
                    "Disc_status": disc_status,
                    "Macula_conf": mac_conf,
                    "Macula_peak_var": mac_spread,  # px spread
                    "Macula_status": mac_status,
                    "Disc_blob_peak_xys_tta": fmt_xys(disc_pts),
                    "Macula_blob_peak_xys_tta": fmt_xys(mac_pts),
                    "Disc_blob_peak_vals_tta": fmt_vals(disc_pts),
                    "Macula_blob_peak_vals_tta": fmt_vals(mac_pts),
                }
            )

    csv_path = out_root / "tta_quality.csv"
    if rows:
