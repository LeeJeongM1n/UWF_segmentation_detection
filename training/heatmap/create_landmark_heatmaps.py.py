...
def process_one_dataset(dataset_dir: Path,
    ...
    for img_path in img_paths:
        ...
        # All GT heatmaps and overlays are generated in one fixed evaluation space.
        # OpenCV resize size order is (width, height).
        bgr = cv2.resize(
            bgr,
    ...
    print(f"fixed output size       : {target_width} x {target_height}")
    print(f"Gaussian sigma          : {sigma} px in fixed output space")
...
def main():
    ...
    p.add_argument("--sigma", type=float, default=20.0, help="Gaussian sigma in fixed output pixels")
    p.add_argument("--target_width", type=int, default=780, help="Fixed GT heatmap width")
    p.add_argument("--target_height", type=int, default=614, help="Fixed GT heatmap height")
    ...
    print(f"[FIXED GT SIZE] {args.target_width} x {args.target_height}")
