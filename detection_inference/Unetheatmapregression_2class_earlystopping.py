...
def denormalize(tensor_chw: torch.Tensor) -> np.ndarray:
    ...
    img = img * IMAGENET_DEFAULT_STD + IMAGENET_DEFAULT_MEAN
...
def main():
    ...
    train_transform = A.Compose([
        A.Resize(height=CONFIG["IMG_SIZE"][0], width=CONFIG["IMG_SIZE"][1], 
                                interpolation=cv2.INTER_CUBIC, mask_interpolation=cv2.INTER_CUBIC),
                                # heatmap : 0~1 Gaussian 연속값 보존
        A.HorizontalFlip(p=0.5),
        A.VerticalFlip(p=0.5),
        A.ColorJitter(brightness=0.2, contrast=0.5, saturation=0.0, hue=0.0, p=1.0),
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            for images, masks, _ in tqdm(valid_loader, desc=f"Epoch {epoch+1}/{args.epochs} [Valid]"):
                ...
                bs = images.size(0)
                for i in range(bs):
                    di, dd = calculate_metrics_binary(probs[i, 0], masks[i, 0], threshold=args.thr) # disc dice, iou
                    mi, md = calculate_metrics_binary(probs[i, 1], masks[i, 1], threshold=args.thr) # macula dice, iou
                    mic_i, mic_d = calculate_micro_metrics(probs[i], masks[i], threshold=args.thr) # micro_ (disc + macula) dice, iou
        ...
        print(f"  LR now: {cur_lr:.6e} | patience {patience_counter}/{args.early_stop_patience}")

        if patience_counter >= args.early_stop_patience:
            print(f"[EARLY STOP] No improvement for {args.early_stop_patience} epochs. Stop at epoch {epoch+1}.")
            break
